...
pub fn main() !void {
    ...
    std.debug.print("=== Voidbox Isolation Mechanisms Showcase ===\n\n", .{});

    // 1. Demonstrate chroot
    std.debug.print("1. Using chroot (legacy):\n", .{});
    const config_chroot = voidbox.JailConfig{
        .name = "chroot-demo",
        .rootfs_path = "/",
    ...
    const outcome1 = try voidbox.launch(config_chroot, allocator);
    std.debug.print("  Exit code: {d}\n\n", .{outcome1.exit_code});

    // 2. Demonstrate pivot_root
    std.debug.print("2. Using pivot_root (secure, modern - DEFAULT):\n", .{});
    const config_pivot = voidbox.JailConfig{
        .name = "pivot-demo",
        .rootfs_path = "/",
    ...
    const outcome2 = try voidbox.launch(config_pivot, allocator);
    std.debug.print("  Exit code: {d}\n\n", .{outcome2.exit_code});

    // 3. Demonstrate bind mounts
    std.debug.print("3. Using bind mounts:\n", .{});
    const config_bind = voidbox.JailConfig{
        .name = "bind-demo",
        .rootfs_path = "/",
    ...
    const outcome3 = try voidbox.launch(config_bind, allocator);
    std.debug.print("  Exit code: {d}\n\n", .{outcome3.exit_code});

    // 4. Demonstrate tmpfs
    std.debug.print("4. Using tmpfs (in-memory filesystem):\n", .{});
    const config_tmpfs = voidbox.JailConfig{
        .name = "tmpfs-demo",
        .rootfs_path = "/",
    ...
    const outcome4 = try voidbox.launch(config_tmpfs, allocator);
    std.debug.print("  Exit code: {d}\n\n", .{outcome4.exit_code});
